	...
	for count in range(2, len(sys.argv)):
		if sys.argv[count].isdecimal() == True:
			...
			if len(api_symbols) == 0:
				...
				api_symbols += '","' + sys.argv[count].upper()
	if api_feed == 'book':
		api_data = '{"event":"subscribe", "subscription":{"name":"%(feed)s", "depth":%(depth)d}, "pair":["%(symbols)s"]}' % {"feed":api_feed, "symbols":api_symbols, "depth":api_number}
	elif api_feed == 'ohlc':
		api_data = '{"event":"subscribe", "subscription":{"name":"%(feed)s", "interval":%(interval)d}, "pair":["%(symbols)s"]}' % {"feed":api_feed, "symbols":api_symbols, "interval":api_number}
	else:
